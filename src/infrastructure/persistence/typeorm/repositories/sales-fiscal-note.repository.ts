...
export class SalesFiscalNoteRepository extends BaseTenantRepository<SalesFiscalNote> {
  constructor(
    @InjectRepository(SalesFiscalNote)
    private readonly salesFiscalNoteRepository: Repository<SalesFiscalNote>,
  ...
  async save(note: SalesFiscalNote): Promise<SalesFiscalNote> {
    ...
    return this.salesFiscalNoteRepository.save(note);
  }

  async saveItem(item: SalesFiscalNoteItem): Promise<SalesFiscalNoteItem> {
    return this.salesFiscalNoteRepository.manager.save(SalesFiscalNoteItem, item);
  }

  async findById(id: string): Promise<SalesFiscalNote | null> {
    const conditions = this.enforceTenantCondition({ id });
    return this.salesFiscalNoteRepository.findOne({ where: conditions });
  }

  async findNotesList(): Promise<SalesFiscalNote[]> {
    return this.salesFiscalNoteRepository.find({
  ...
  async findNoteDetails(id: string): Promise<any | null> {
    const note = await this.salesFiscalNoteRepository.findOne({
    ...
    const items = await this.salesFiscalNoteRepository.manager
      .createQueryBuilder(SalesFiscalNoteItem, 'item')
